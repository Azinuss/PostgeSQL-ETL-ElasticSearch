...
from api.v1.schemas.shop import  ShopSchema
from api.v1.schemas.shop import ShopPurchaseRequestSchema, ShopPurchaseResponseSchema
from api.v1.schemas.shop import ShopGetByNameRequestSchema, ShopGetByNameResponseSchema
...
async def list_product(shop_persistence :ShopPersistence = Depends(shop_persistence_dependency)) -> List[ShopSchema]:
    ...
    return [ShopSchema(id=product.id ,name=product.name, description=product.description,
                    count_left=product.count_left, create_time=product.create_time, update_time=product.update_time) for product in products]


@router.get("/{groupe_id}", summary="Get product", description="Get product by id",
            response_model=Union[ShopSchema,None])
async def get_product_by_id(product_id: int,
                    shop_persistence :ShopPersistence = Depends(shop_persistence_dependency)) -> Union[ShopSchema,None]:
...
async def buy_product(buy_request: ShopPurchaseRequestSchema,
                       shop_persistence :ShopPersistence = Depends(shop_persistence_dependency)) -> ShopPurchaseRequestSchema:
...
async def get_product_by_name(search_request: ShopGetByNameRequestSchema,
                       shop_persistence :ShopPersistence = Depends(shop_persistence_dependency)) -> ShopGetByNameRequestSchema:
