...
class PostgresShopPersistence(ShopPersistence):
    ...
    def get_product_by_name(self, product_name: str) -> Shop:
        ...
        if shop_model["hits"]["max_score"]:
            return Shop(id=shop_model["hits"]["hits"][0]["_source"]["id"], name=shop_model["hits"]["hits"][0]["_source"]["name"], description=shop_model["hits"]["hits"][0]["_source"]["description"],
                        count_left=shop_model["hits"]["hits"][0]["_source"]["count_left"], create_time=shop_model["hits"]["hits"][0]["_source"]["create_time"], update_time=shop_model["hits"]["hits"][0]["_source"]["create_time"])
        else:
